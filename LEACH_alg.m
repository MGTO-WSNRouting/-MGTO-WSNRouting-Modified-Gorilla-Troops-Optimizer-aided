function [y, norm_Energy, CLUSTERHS, CH, GlobalMins, GlobalMin, ct, bs, out]=LEACH_alg(n,p,ETX,ERX,Efs,Emp,EDA,rmax, no_sol,dim_sol,iteration_count,S_in,objfun_Cluster)
global S alg v num_of_sink
S=S_in;
%Computation of do
do=sqrt(Efs/Emp);
...
for rm=0:rmax - 1
    ...
    [GlobalMin(v),GlobalMins{v},GlobalParams,ct(v)]=feval(alg,initsol,objfun_Cluster,Xmin,Xmax,iteration_count);
    
    GlobalParams(isnan(GlobalParams)) = 1;
    GlobalParams = round(GlobalParams);
    GlobalParams = check_obj(n,GlobalParams);
    bs{v} = GlobalParams;
    [ObjVal,CH{v},out{v}]=feval(objfun_Cluster,bs{v});
        ...
        if(S(i).E>0)
            ...
            if ( (S(i).G)<=0)
                ...
                for sink_iter = 1:num_of_sink
                    distances(sink_iter) =sqrt( (S(i).xd-(S(n+sink_iter).xd) )^2 + (S(i).yd-(S(n+sink_iter).yd) )^2 );
                end
    ...
    for i=1:1:n
        if ( S(i).type=='N' && S(i).E>0 )
            if(cluster-1>=1)
                for sink_iter = 1:num_of_sink
                    min_dis=sqrt( (S(i).xd-S(n+sink_iter).xd)^2 + (S(i).yd-S(n+sink_iter).yd)^2 );
                    %                 min_dis2=sqrt( (S(i).xd-S(n+2).xd)^2 + (S(i).yd-S(n+2).yd)^2 );
                    %                 min_dis3=sqrt( (S(i).xd-S(n+3).xd)^2 + (S(i).yd-S(n+3).yd)^2 );
                    min_dis_cluster=1;
                    for c=1:1:cluster-1
                        minimum1=min(min_dis, sqrt( (S(i).xd-C(c).xd)^2 + (S(i).yd-C(c).yd)^2 ) );
                        %                     minimum2=min(min_dis2, sqrt( (S(i).xd-C(c).xd)^2 + (S(i).yd-C(c).yd)^2 ) );
                        %                     minimum3=min(min_dis3, sqrt( (S(i).xd-C(c).xd)^2 + (S(i).yd-C(c).yd)^2 ) );
                        temp = minimum1;
                        %                     min_dis = min(min_dis);
                        if ( temp<min_dis)
                            min_dis=temp;
                ...
                temp;
                if (temp>do)
                    S(i).E=S(i).E- ( ETX*(4000) + Emp*4000*( temp * temp * temp * temp));
                end
                if (temp<=do)
                    S(i).E=S(i).E- ( ETX*(4000) + Efs*4000*( temp * temp));
                end
                %Energy dissipated
                if(temp>0)
